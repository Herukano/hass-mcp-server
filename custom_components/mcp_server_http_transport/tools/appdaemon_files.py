...
def _new_backup(root: Path) -> tuple[Path, list[str]]:
    ...
    if backup_base.is_symlink() or not backup_base.resolve(strict=False).is_relative_to(
        root.resolve()
    ):
...
@register_tool(
    ...
    "List regular files under the fixed AppDaemon apps root.",
...
async def list_appdaemon_files(hass: HomeAssistant, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        def work():
            return [
                {"path": _relative(p), "size": p.stat().st_size, "sha256": _sha(p.read_bytes())}
                for p in _app_files(_root())
            ]
...
@register_tool(
    "get_appdaemon_file",
    "Read a file relative to the fixed AppDaemon apps root.",
            ...
            "content": [
                {
                    "type": "text",
                    "text": json.dumps(
                        {"success": True, "backup": _relative(backup), "files": files}
                    ),
                }
            ]
        }
            ...
            return [
                {
                    "timestamp": p.name,
                    "path": _relative(p),
                    "files": [x.relative_to(p).as_posix() for x in _app_files(p)],
                }
                for p in sorted(base.iterdir(), reverse=True)
                if p.is_dir() and _BACKUP_TS_RE.fullmatch(p.name) and not p.is_symlink()
            ]
...
async def restore_appdaemon_backup(
    ...
    try:
        ...
        if not _BACKUP_TS_RE.fullmatch(timestamp):
            raise ValueError("Invalid backup timestamp")

        def work():
            root = _root()
            source = root / _BACKUP_DIR_NAME / timestamp
            if (
                not source.is_dir()
                or source.is_symlink()
                or not source.resolve().is_relative_to((root / _BACKUP_DIR_NAME).resolve())
            ):
                raise ValueError("Backup does not exist")
            pre, _ = _new_backup(root)
            restored = []
            for item in _app_files(source):
                rel = item.relative_to(source)
                target = _safe_path(rel.as_posix())
                _atomic_write(target, item.read_bytes(), stat.S_IMODE(item.stat().st_mode))
                restored.append(rel.as_posix())
            return {
                "success": True,
                "backup": _relative(source),
                "pre_restore_backup": _relative(pre),
                "restored": restored,
            }
